...
flowchart TD
    ...
    K --> L[Set-Cookie: access_token=JWT\nHttpOnly · Secure · SameSite=Strict]
...
flowchart TD
    ...
    Q --> R
    R --> S[INSERT refresh_tokens\nstore SHA256 hash · TTL = 7 days]
    S --> T[Set-Cookie: access_token=JWT\nHttpOnly · Secure · SameSite=Strict\nSet-Cookie: refresh_token\nHttpOnly · Secure · Path=/auth/refresh]
    T --> U[302 Redirect → /\napplication home]
    U --> End3([🟢 End])
```
...
flowchart TD
    ...
    CLEANUP -.->|"Periodic scan\nDELETE status=draft AND age > 24h"| R
...
> **Note:** The deferred embedding gate (P3.0) runs inline on first query when the document status is `draft`. The JWT is read from the HTTP-only cookie — no token is passed in the request body or query string.

```mermaid
flowchart TD
    Start([🟢 Start]) --> A[POST /chat\nbody: question · documentID · optional sessionID · top_k]
    A --> B{JWT valid?\nfrom cookie}
    B -->|No| C[Return 401]
    C --> End1([End])

    B -->|Yes| BV{Email verified?}
    BV -->|No| BVE[Return 403\nEmail not verified]
    BVE --> End1
    ...
    DOC{Document\nstatus = draft?}
    DOC -->|Yes — deferred\nembedding phase| DA[Load chunks\nfrom chunks/ filesystem]
    DA --> DB[Embed each chunk\nOllama nomic-embed-text:v1.5\n→ float32 768-dim vectors]
    DB --> DC[Upsert vectors\ninto Qdrant\nwith documentID + userID payload]
    DC --> DD[UPDATE documents\nSET status = ready\nchunk_count = N]
    DD --> I

    DOC -->|No — status = ready\nskip to query| I

    I[INSERT user message\nrole=user · content=question]
    I --> J[Ollama.Embed question\nnomic-embed-text:v1.5\n→ float32 768-dim]
    J --> K[Qdrant.Search\nvector · top_k · documentID filter]
    K --> L[Assemble prompt:\nsystem + chunks + question]
    L --> M[Set response headers:\nContent-Type: text/event-stream\nCache-Control: no-cache\nX-Accel-Buffering: no]
    M --> N[Ollama.StreamChat\nqwen:3.5-4b · stream=true]
    N --> O{More tokens?}
    ...
    style DA fill:#f3e5f5,stroke:#6a1b9a,stroke-width:1px
    style DB fill:#f3e5f5,stroke:#6a1b9a,stroke-width:1px
    style DC fill:#f3e5f5,stroke:#6a1b9a,stroke-width:1px
    style DD fill:#f3e5f5,stroke:#6a1b9a,stroke-width:1px
```
...
> **Note:** Vai uses a single shared Qdrant collection (`documents`). Deletion removes all vectors where the payload `userID` matches — it does not drop an entire collection.
...
flowchart TD
    ...
    B -->|Yes| D["Qdrant.Delete\nfilter: payload.userID = userID\nfrom shared 'documents' collection"]
    D --> E{Qdrant delete\nsuccessful?}
    E -->|No — log error,\ncontinue| F[Log error, proceed\ndo not block deletion]
    E -->|Yes| G[DELETE FROM users\nWHERE id = userID]
    F --> G
    G --> H[PostgreSQL CASCADE deletes:\ndocuments · conversations\nmessages · tokens · oauth_accounts]
...
flowchart TD
    ...
    P --> Q[Set-Cookie: access_token=JWT\nHttpOnly · Secure · SameSite=Strict\nSet-Cookie: refresh_token\nHttpOnly · Secure · Path=/auth/refresh]
    Q --> R[Return 200 OK\nuser profile]
    R --> End5([🟢 End])
```