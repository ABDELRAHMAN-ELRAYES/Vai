...
export function FileCard({ entry, data, onRemove }: FileCardProps) {
  const [preview, setPreview] = useState<string | null>(null);

  const name = entry?.file?.name ?? data?.name ?? "unknown";
  const size = entry?.file?.size ?? data?.size ?? 0;
  const type = entry?.file?.type ?? data?.type ?? "";
          ...
          <div className="min-w-0 relative">
            ...
            <div className="flex gap-2 items-center mt-1">
              ...
              <p className="text-[10px] text-gray-400 font-mono leading-tight mt-0.5">
                {tooLarge ? "too large" : formatSize(size)}
              </p>
            </div>
